...
public class NGramsStrategy implements DetectionStrategy {
	...
	public boolean isEncompassed(ArrayList<Run> runList, int f1start, int f1end, int f2start, int f2end) {
		for (int i = 0; i < runList.size(); i++) {
			if ((f1start >= runList.get(i).getFile1Indicies().getKey()) && (f1end <= runList.get(i).getFile1Indicies().getValue())) {
				if ((f2start >= runList.get(i).getFile2Indicies().getKey()) && (f2end <= runList.get(i).getFile2Indicies().getValue())) {
					return true;
				}
			}
		}
